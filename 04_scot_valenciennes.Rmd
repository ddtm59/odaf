...
choix_scot <- "SCoT de Valenciennes"
choix_dept <- "59"
...
- 861 Ha artificialisés, soit plus de 16% des surfaces artificialisées du département (2^e^ position);
- 530 Ha ont été artificialisés à destination de l'habitat, soit 65% ;
- 281 Ha l'ont été à destination de l'activité, soit 35% ;
- Le rapport de force entre artificialisation liée à l'habitat et à l'activité se place dans la tendance départementale ;
- L'artificialisation est en baisse constante sur l'ensemble de la période 2009-2018 en ce qui concerne l'habitat et l'activité (respectivement -67% et -58% par rapport à 2009) ;
- Le `r choix_scot` a artificialisé proportionnellement autant que son poids démographique départemental ;
- L'artificialisation médiane du SCoT est de 7,6 Ha par commune entre 2009 et 2018, contre 4,8 Ha au niveau départemental ;
...
- L'artificialisation en matière d'activité fait partie des plus fortes du département (2^e^place);
- Le `r choix_scot` artificialise de l'ordre de 1,3 fois son poids relatif en matière d'emplois et 1,2 fois son poids relatif en ménages ;
- Pour chaque hectare artificialisé à destination de l'activité sur la période 2012-2017, le `r choix_scot` a perdu 18 emplois (taux négatif le moins important) ;
- Le SCOT se classe en 3^e^ position en termes d'artificialisation en matière d'habitat ;
- Pour chaque hectare artificialisé à destination de l'habitat sur la période 2012-2017, le `r choix_scot` a connu un gain de 15 ménages (taux se trouvant dans la moyenne haute).
...
La CA de la Porte du Hainaut a artificialisé plus de la moitié des surfaces (`r round(455/(455+407) *100 , digits = 0)`%) suivie par la CCPM avec 210 Ha (`r round(0.47*100 , digits = 0)`%).

Cet équilibre est bien entendu lié à la taille respective des EPCI ainsi que de leur population et tissus économiques propres.
...
On note que la CAVM (territoire le plus peuplé) a artificialisé proportionnellement moins que son poids démographique (55% contre 47%). À l'inverse, la CAPH a connu une artificialisation plus forte au regard de sa population. 
...
On note une divergence entre les la CAVM et la CAPH qui connaît une artificialisation plus proche de l'évolution des ménages.
...
+ On constate également de grandes disparités annuelles quant à l'évolution du flux : on constate des pics d'artificialisation ponctuellement notamment pour la CAPH.
...
On constate alors que les écarts de tendances entre EPCI également faibles. Chaque EPCI a artificialisé près de 10 Ha par commune en moyenne. Les communes de la CAVM ont artificialisé légèrement plus (11.6 Ha), alors que les communes des autres EPCI ont artificialisé autour de 9.9 Ha.
...
- Les valeurs pour les aires de Bouchain et de Somain sont à analyser avec précaution, car ces aires ne sont composées que d'une seule commune chacune ;
- Les communes se situant dans l'aire de Lille artificialisent plus que celles de l'aire de Valenciennes - médiane à 9,5 Ha pour Lille contre 7.9 pour Valenciennes ;
- Les communes en dehors des aires artificialisent moins que celles se trouvant dans les aires.
...
- Les communes-centres artificialisent beaucoup plus que le reste des autres types de communes du territoire ;
- Les communes de la couronne artificialisent plus que celle se situant en périphérie immédiate des pôles ;
- Les communes des pôles autre que la commune-centre connaissent une forte disparité quant à leur taux d'artificialisation ;
- Quelques communes *exceptions* artificialisent beaucoup plus que la tendance observée pour leur type (Saint-Amand-les-Eaux, Prouvy, La sentinelle, Hordain)
...
La cartographie permet de distinguer clairement des poches de communes ayant plus artificialisé que les autres, notamment pour les communes en périphérie de Valenciennes ainsi que Saint-Amand-les-Eaux.
...
- Les communes les plus peuplées qui artificialisent moins que leur poids en population se trouvent à droite de la droite grisée. À titre d'exemple la ville de Valenciennes représente plus de 12% de la population, mais moins de 2% de l'artificialisation ;
- Les communes avec une faible population et une faible artificialisation ;
- Les communes se trouvant dans la partie supérieure gauche comme Prouvy ou encore La Sentinelle qui, bien que ne représentant pas une part importante de la population de l'arrondissement, ont artificialisé proportionnellement beaucoup plus. *L'analyse plus fine de l'artificialisation par type de destination permettra de comprendre s'il s'agit plutôt de territoires ayant une dynamique plutôt orientée vers l'activité* ;
- On peut également noter les divergences de dynamiques pour des communes proches en matière de poids démographique, comme le couple Denain/Saint-Amand-les-Eaux qui, bien que représentant une part similaire de la population (autour de 5%), n'ont pas artificialisé de la même manière : près de 7% pour Saint-Amand contre 2% pour Denain.
...
On peut par exemple noter que 25% des communes du `r choix_scot` ont artificialisé plus de 13 Ha sur la période 2009-2018.
...
dcomdept_ref_ti_scot %>% 
  ...
  labs(x = "Ha artificialisés par commune",
       ...
       caption = "Source : CEREMA, Traitement : DDTM 59",
       fill = "")+
         geom_vline(xintercept=13,color="grey")+
...
Ces cartographies permettent de mieux cerner les tendances observées dans la version à la maille communale. On observe très clairement la périurbanisation des périphéries des pôles, notamment au *Sud-Ouest* de Valenciennes. 
...
- Valenciennes et Saint-Saulve ont perdu de nombreux emplois et gagné des ménages ;
- Les communes se trouvant dans les pôles connaissent des situations très hétérogènes ;
- Peu de communes ont perdu des ménages : Raismes, Anzin, Saint-Amand-les-Eaux, Condé-sur-l'Escaut, etc.
...
On note une plus forte spécialisation de la CAVM en matière d'activité *La valeur pour le département étant de 34%*.
...
On constate que les tendances sont similaires et suivent la même dynamique de baisse en ce qui concerne l'habitat. En revanche, il est plus difficile de conclure en matière d'activité même si la tendance semble à la baisse, on observe de nombreuses années avec des taux d'artificialisation plus élevé que la tendance notamment pour la CAPH.
...
- On distingue alors qu'une la tendance baissière de l'habitat peut se généraliser sur l'ensemble des types de communes.

- On observe une baisse du rythme d'artificialisation liée à l'activité concernant les communes du pôle de Valenciennes ainsi que celles situées dans la couronne de l'aire de Lille.

- On retrouve également les pointes ponctuelles d'artificialisation liée à l'activité constatée dans le point précédent pour les communes de la couronne de Valenciennes ainsi que pour les communes se situant en dehors des aires.
...
Ces graphiques mettent à nouveaux en avant le phasage - ou non - entre les types de destination tout en pointant la divergence en matière de rythme : plus le temps passe, plus l'artificialisation liée à l'activité ralentit.
...
- Valenciennes et sa sa périphérie concentre une grande partie de l'artificialisation liée à l'activité ;
- Saint-Amand-les-Eaux et Hordain ont également beaucoup artificialisé ;
...
- Le Sud-Ouest de Valenciennes a particulièrement peu artificialisé.
...
- Les communes ont une forte tendance à artificialiser préférentiellement à destination de l'habitat ;
- Quelques communes se distinguent en matière de spécialisation de leur artificialisation :
  - Prouvy, Rouvignies, Hordain et la Sentinelle qui ont une forte spécialisation en matière d'activité ;
  - Wallers, Condé-sur-l'Escaut, Saint-Saulve qui ont une forte spécialisation en matière d'habitat.
...
On retrouve les mêmes dynamiques que pour la répartition démographique avec une accentuation des écarts entre les 2 EPCI.

On note que la CAVM (territoire avec le plus d'emplois) a artificialisé proportionnellement moins que son poids en emplois (65% contre 52%). À l’inverse, la CAPH a connu une artificialisation plus forte au regard de son poids en emplois (35% contre 48%).
...
- À l'exception de Bouchain et Somain, on note une certaine homogénéité des valeurs médianes d'artificialisation liée à l'activité ;
- De nombreuses valeurs extrêmes sur l'aire de Valenciennes ;
- Une relative homogénéité des surfaces communales consommées par aire (médian et inter-quartile) ;
- Un plus grand écart inter-quartile sur l'aire de Lille.
...
On constate que les communes ayant le plus artificialisé pour l'activité se trouve dans le pôle des aires, ainsi que les communes-centres.

Les communes se trouvant en dehors du pôles (couronne ou hors des aires), à l'exception des quelques cas sortant de la boîte à moustaches, n'ont pratiquement pas consommé d'ENAF à destination de l'activité.
...
- Les communes centre, bien que concentrant près de 28% des emplois, n'artificialisent que 2% des espaces liés à l'activité ;
- Les autres communes du pôle principal représentent qu'une part modeste de l'emploi (23%), ainsi qu'une part relativement modeste de l'artificialisation (17%) ;
- Les communes se trouvant dans la couronne et hors de l'aire représentent la part la plus importante de l'emploi et de l'artificialisation, selon un rapport de forces en faveur de l'artificialisation : elles ont consommées proportionnellement à leur poids en emplois, plus de surfaces à destination de l'activité ;
- On peut faire le même constat concernant les communes hors des aires d'attraction, en notant que le ratio est encore plus élevé 1:10.
...
On peut par exemple noter que **Seulement 32% des communes du `r choix_scot` ont artificialisé plus de 2 Ha sur la période 2009-2018.**
...
dcomdept_ref_ti_scot %>% 
  ...
  labs(x = "Ha artificialisés par commune",
       ...
       fill = "")+
         ...
         geom_hline(yintercept=0.68,color="grey")+
  scale_y_continuous(labels = scales::percent)
...
- Valenciennes qui représente moins de 30% des emplois n'a artificialisé environ 1% des surfaces liées à l'activité ;
- A l'inverse Prouvy et Rouvignies ont artificialisé environ 6 fois plus que leur part d'emplois au sein du SCoT.
...
- au *Sud-Ouest* de Valenciennes ;
- au *Sud* de Hordain ;
- en périphérie de nombreuses agglomérations.
...
Les dynamiques en matière de ménages sont similaires aux rapports de forces démographique et d'emplois avec une accentuation des écarts entre les 2 EPCI.

On note que la CAVM (territoire avec le plus d’emplois) a artificialisé proportionnellement moins que son poids en ménages (57% contre 44%). À l’inverse, la CAPH a connu une artificialisation plus forte au regard de son poids en ménages (43% contre 56%).
...
La médiane oscille entre 1,7 Ha par commune pour les communes hors d'aires d'attraction et 6Ha pour les aires de valenciennes et Lille.

On peut distinguer deux types d'aires :

- Lille et Valenciennes ayant une médiane et rapport inter-quartile important impliquant donc plus d'artificialisation, mais une plus grande disparité en fonction des communes avec quelques valeurs extrêmes chacune ;
- Les autres aires ont les mêmes dynamiques avec des valeurs d'artificialisation se trouvant dans les premiers quartiles des grandes aires.
...
Assez logiquement, on retrouve un gradient d’artificialisation en fonction de la distance à la commune centre, ainsi que la grande diversité d’artificialisation des communes de la couronne.  **Plus on s'éloigne du centre de l'aire d'attraction, plus les communes ont une tendance à moins artificialiser à destination de l'habitat.**
*Attention toutefois, le rapport de forces entre communes de la couronne et commune du pôle est inversé.*

Quelques communes de la couronne ont une artificialisation sortant de la boîte à moustaches telles que Saint-Amand-les-Eaux et Wallers.
...
On observe des tendances identiques du couple emplois-activité :

- Les communes-centres, bien que concentrant près de 16 % des ménages, n’artificialisent que 3% des espaces liés à l’habitat ;
- Les autres communes du pôle principal suivent la même dynamique dans une proportion plus faible ;
- Les communes se trouvant dans la couronne représentent plus de la moitié des ménages, mais 79% de l'artificialisation ;
- Les communes hors des aires d'attractions artificialisent relativement beaucoup plus que leur poids relatif en matière de ménages - *plus de 2 fois*.
...
**50% des communes ont artificialisé moins de 5 Ha durant les 10 dernières années.**
...
- Quelques communes situées dans la partie en haut à gauche du graphique ont un rapport entre leur poids dans l'artificialisation liée à l'habitat et leur taux de ménages très élevé. Cet écart est relativement faible au regard des autres SCOT ;
- Valenciennes représentant 15% des ménages, n'a artificialisé à destination de l'habitat que 2% des surfaces artificialisées du SCOT.
...
- Un grand nombre de communes-centres et a sein des pôles ont toutes perdu des emplois ;
...
- Maubeuge se distingue nettement en matière de perte d'emplois, mais avec également une artificialisation liée à l'activité très faible ;
- Prouvy et Hordain se distinguent avec une grande artificialisation liée à l'activité et avec peu d'emplois perdus ou gagnés ;
- On peut noter également le cas de Rouvignies qui a artificialisé plus de 25Ha et qui dans le même temps a créée plus 500 emplois ;
...
- Le *Sud* de Valenciennes a beaucoup artificialisé ;
- Toutefois, cette carte fait ressortir de grandes disparités dans ces dynamiques :
  - Quelques communes connaissent une efficacité positive (avec un maximum à 152 emplois par Ha artificialisé), alors que d'autres se stabilisent ;
...
*Les communes-centres n'apparaissent pas dans ce diagramme car elles n'ont pas suffisamment artificialisé pour que le taux d'efficacité ait un sens.*

On observe ainsi que les communes du pôle principal, bénéficiant du plus fort taux d'emplois connaissent une grande variabilité liée à l'évolution du taux d'emplois, mais ont - de manière statistique - une meilleur efficacité que les communes de la couronne.
...
- Les communes des pôles ont plus généralement gagné des habitants (seules 4 se trouvent sous l'axe avec des pertes plus faibles);
- Bien qu'il n'y ait pas de corrélation nette, on remarque un léger lien entre la surface artificialisée et le nombre de ménages gagné par commune ;
- Petite-Forêt et Saint-Amand-les-Eaux ont artificialisé environ 15Ha de surface à destination de l'habitat, mais la première voit son nombre de ménages augmenter de presque 100 alors que la seconde en a perdu.
...
- La partie *Est* de Valenciennes dispose de plus de communes avec un taux positif assez élevé.
...
- Les communes du pôle principal ont en valeur médiane, le taux d'efficacité le plus élevé ;
- L'efficacité diminue à mesure que l'on s'éloigne de la commune-centre ;
- L'efficacité est négative pour les communes centres du SCOT ;
- L'écart inter-quartile est le plus important pour les communes des pôles.
