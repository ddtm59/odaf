...
-   568 Ha artificialisés, soit plus de 10% des surfaces artificialisées du département ;
-   330 Ha ont été artificialisés à destination de l'habitat, soit 63% ;
-   193 Ha l'ont été à destination de l'activité, soit 37% ;
-   Le SCOT connaît une spécialisation de son artificialisation en faveur de l'activité 37% (contre 33% au niveau départemental) ;
-   L'artificialisation est en baisse constante sur l'ensemble de la période 2009-2018 (environ -80% par rapport à 2009 pour l'habitat et l'activité ) ;
-   Le `r choix_scot` a artificialisé près de `r round(10/6,digits=1)` fois son poids démographique ;
-   L'artificialisation médiane du SCoT est de 3,5 Ha par commune entre 2009 et 2018, contre 4,8 Ha au niveau départemental ;
...
-   L'artificialisation en matière d'habitat et d'activité font parties des plus faibles du département ;
...
-   Pour chaque hectare artificialisé à destination de l'activité sur la période 2012-2017, le `r choix_scot` a perdu 24 emplois ;
-   Pour chaque hectare artificialisé à destination de l'habitat sur la période 2012-2017, le `r choix_scot` a connu un gain de 10 ménages ;
...
Les EPCI du `r choix_scot` ont des bilans d'artificialisation très contrastés : La CA de Cambrai a artificialisé plus de la moitié des surfaces (`r round(320/(320+190+58) *100 , digits = 0)`%) suivie par la CA du Caudrésis et Catésis avec 190 Ha (`r round(190/(320+190+58) *100 , digits = 0)`%). Enfin la CC du Pays du Solesmois a artificialisé près de 60 Ha (`r round(58/(320+190+58) *100 , digits = 0)`%).
...
dcomdept_ref_ti %>%
  ...
  theme(plot.subtitle = element_text(hjust = -5.5))
```

On note alors que la CAC (territoire le plus peuplé) a artificialisé proportionnellement plus que son poids démographique (56% contre 51%). À l'inverse, la CACC a connu une artificialisation plus faible au regard de sa population. Quant à la CCPS, les rapports de forces semblent être assez équilibrés.
...
-   On retrouve les rapports de forces décrits dans la première figure avec une très forte prégnance de la CAC.
...
On constate alors que les écarts de tendances entre EPCI sont plus faibles. Chaque EPCI a artificialisé autour de 5 Ha par commune en moyenne. Les communes de la CA de Cambrai ont artificialisé légèrement plus (6 Ha), alors que les communes des deux autres EPCI ont artificialisé légèrement sous les 5 Ha.

On note toutefois que les valeurs extrêmes sont en grande majorité au sein de la CAC. Certains communes de la CACC ont également beaucoup artificialisé.
...
-   L'aire de Solesmes connaît la dynamique d'artificialisation la plus élevée avec une médiane à près de 7,5 Ha.
-   A l'exception de Solesmes, le reste des aires d'attraction a artificialisé selon la même dynamique (\~ 3 Ha par commune)
...
-   Seul l'aire de Cambrai connaît un grande amplitude en matière d'artificialisation (due en grande partie à la superficie couverte par l'aire.)
...
-   Les communes centres artificialisent plus que le reste du territoire (environ entre 2 et 4 fois plus).
-   On constate que les communes centres ont une artificialisation très différentes allant de moins de 10 Ha à plus de 4 fois cette valeur (Caudry).
...
La cartographie permet de distinguer clairement des poches de communes ayant plus artificialisé que les autres, notamment pour les communes en périphérie de Cambrai ainsi que pour quelques communes reparties selon les principaux pôles du `r choix_scot`.
...
-   Les communes les plus peuplées qui artificialisent moins que leur poids en population se trouvent à droite de la droite grisée. À titre d'exemple la ville de Cambrai représente 20% de la population, mais moins de 4% de l'artificialisation ;
-   Les communes avec une faible population et une faible artificialisation ;
-   Les communes se trouvant dans la partie supérieure gauche comme Tilloy-lez-Cambrai ou encore Sancourt qui, bien que ne représentant pas une part importante de la population de l'arrondissement, ont artificialisé un ordre de grandeur au-dessus. *L'analyse plus fine de l'artificialisation par type de destination permettra de comprendre s'il s'agit plutôt de territoires ayant une dynamique plutôt orientée vers l'activité.*
...
Ces cartographies permettent de mieux cerner les tendances observées dans la version à la maille communale. On observe très clairement la périurbanisation des périphéries des pôles, notamment autour de Caudry ou sur la frange *Ouest* de Cambrai.
...
-   Cambrai a perdu un nombre - relativement - important de ménages et d'emplois durant la période 2012-2017 ;
-   Toutes les communes-centres ont perdu des emplois et deux d'entre-elles ont perdu également des ménages ;
-   La situation des communes "pôle principal" est très contrastée en fonction des communes, notamment en matière d'emplois ;
-   Certaines communes de la couronne ont connu de fortes baisses en matière d'emplois.
...
tm_scot_men1217 <- dcomdept_ref_sf_scot %>% 
  tm_shape()+
    tm_borders(col="grey")+
    tm_fill(col="men1217", palette = "RdBu",title = "Evolution des ménages \nentre 2012 et 2017")+
  tm_layout(frame = F,legend.outside = F)+
  dcomdept_ref_sf_scot %>% filter(LIB_CATEAAV2020=="Commune-centre") %>% 
  tm_shape()+
  tm_text(text = "IDCOMTXT",size = 0.7)+
...
La carte ci-dessus reprend les données du graphiques précédent dans une version géolocalisée, permettant ainsi de mieux discerner les tendances spatiales en matière d'évolution socio-économiques.

Cela permet de distinguer des poches de pertes de ménages (Sud du Cateau-Cambrésis) ou encore une couronne d'évolution de l'emploi à la hausse autour de la ville de Cambrai ainsi qu'en frange Ouest de Caudry.
...
Les deux graphiques ci-dessus reprennent les données présentées en début de partie, en incluant cette fois-ci la ventilation entre les deux destinations.

-   On note une plus forte spécialisation de la CAC en matière d'activité. *La valeur pour le département étant de 34%*;
-   La CCPS est très nettement orientée vers une dynamique résidentielle.
...
On note que les tendances pour les deux destinations sont très proches et suivent la même dynamique de baisse. On observe également que les pics d'artificialisation sont - *presque* - systématiquement en phase.
...
On note que de nombreux territoires ont en réalité une dynamique stable. **Les communes de la couronne de Cambrai emportent à elles seules les éléments du graphiques précédent.**
...
Ces graphiques mettent à nouveaux en avant le phasage entre les types de destination, tout en pointant la divergence en matière de rythme : plus le temps passe, plus l'artificialisation liée à l'activité ralentit.
...
-   Cambrai et sa frange *Ouest* concentre une grande partie de l'artificialisation liée à l'activité ;
-   Caudry a également beaucoup artificialisé ;
-   Quelques communes (Le Cateau-Cambrésis, Gouzeaucourt, Iwuy, etc. ) ont relativement artificialisé.
...
-   Caudry a artificialisé plus de surfaces pour l'habitat (11 Ha) que Cambrai malgré sa taille plus faible et a consommé plus de 3 fois de surface pour l'activité avec près de 27 Ha.
-   Cambrai a une dynamique équilibrée avec autant de surface pour l'habitat que pour l'activité.
-   Quelques communes comme Iwuy, Proville, Gouzeaucourt, Haynecourt, Sailly-lez-Cambrai ont artificialisé autant ou plus de surfaces liées à l'activité que Cambrai malgré des tailles de communes assez modestes.
-   De nombreuses communes se situent sur l'axe ou proche de l'axe vertical, ce qui signifie qu'elles n'ont qu'une dynamique d'aménagement résidentielle.
-   Cela ne nous dit pas si ces artificialisations sont adaptées aux emplois ou ménages accueillis sur ces communes.
...
Afin de mieux comprendre les dynamiques inter-territoires en matière d'artificialisation liée à l'activité, on peut de nouveau analyser le phénomène au regard des mailles proposées précédemment : EPCI, Aires d'attractions et type de communes, communale et infra-communale.
...
-   Une sur-représentation de la part de la CAC en matière d'artificialisation ;
-   Une relative densité de l'artificialisation liée à l'emploi à l'échelle de la CACC ;
-   Une proportion équilibrée en ce qui concerne la CCPS.
...
-   Un taux d'artificialisation plus élevé pour l'aire de Solesmes ;
-   De nombreux extrêmes sur l'aire de Cambrai ;
-   Une relative homogénéité des surfaces communales consommées par aire.
...
On retrouve à nouveau le gradient d'artificialisation en fonction de la distance à la commune centre, ainsi que la grande diversité d'artificialisation des communes centres. **Plus une commune s'éloigne du centre du pôle d'attractivité, moins la commune artificialise de espaces** - statistiquement.
...
-   Les communes centre, bien que concentrant près de 60% des emplois, n'artificialisent qu'un quart des espaces liés à l'activité ;
-   Les autres communes du pôle principales ne représentent que très peu d'emplois et d'artificialisation. La part de l'artificialisation est toutefois 50% plus importante que l'emploi ;
-   Les communes se trouvant dans la couronne représentant seulement un tiers des emplois, artificialisent plus de 60% des espaces liés à l'activité.
...
On peut par exemple noter que **75% des communes du `r choix_scot` ont artificialisé moins de 1 Ha sur la période 2009-2018.**
...
-   Cambrai qui représente plus de 35% des emplois n'a artificialisé moins de 5% des surfaces liées à l'activité ;
-   Caudry et le Cateau-Cambrésis ont artificialisé proportionnellement autant que leur part d'emplois au sein du SCoT ;
-   Les communes se trouvant proches de l'axe des ordonnées et dans la CAC (Tilloy, Sancourt, etc.) ont artificialisé proportionnellement beaucoup plus que leur part en termes d'emplois au sein du SCoT.
...
On observe quelques secteurs spécifiques avec un fort taux d'artificialisation :

-   au *Nord-Ouest* de Cambrai ainsi que plus légèrement au *Sud-Ouest* ;
-   à l'*Est* de Caudry ;
-   à l'*Est* du Cateau-Cambrésis.

L'ensemble de ces carreaux se situent en périphérie des tâches urbaines.
...
Afin de mieux comprendre les dynamiques inter-territoires en matière d'artificialisation liée à l'habitat, on peut de nouveau analyser le phénomène au regard des mailles proposées précédemment : EPCI, Aires d'attractions et type de communes, communale et infra-communale.
...
Les dynamiques en matière de ménages sont différentes des dynamiques en matières de population et d'emplois. On observe une relative égalité en proportions - *avec une légère sur-représentation des ménages sur l'artificialisation* - entre la répartition des ménages et du poids dans l'artificialisation liée à l'habitat. On note tout de même une sur-représentation de l'artificialisation liée à l'habitat au sein de la CCPS.
...
# dcomdept_ref_ti_scot %>% 
#   group_by(lib_aav2020) %>% 
# summarise(median(arthab0919,na.rm = T)/10000)
```

On peut ainsi noter une grande disparité des dynamiques entre communes de chaque aire. La médiane oscille entre 3,2 Ha par commune pour l'aire de Valenciennes et 1,2 Ha pour l'aire de Caudry.

Les aires de Cambrai et de Caudry se distinguent par des communes se trouvant au delà des valeurs types de la boite à moustaches telles que Awoingt, Les Rues-des-Vignes, Cambrai ou encore Caudry.

L'écart inter-quartile pour Douai et Solesmes est faible, mais cela est lié au fait qu'il y a très peu de communes dans ces aires au sein du `r choix_scot`.
...
On observe les mêmes tendances que pour le couple emplois-activité :

-   Les communes-centres, bien que concentrant près de 40 % des ménages, n'artificialisent moins de 10% des espaces liés à l'habitat ;
-   Les autres communes du pôle principal ne représentent que très peu de ménages (5%) et d'artificialisation (3%) ;
-   Les communes se trouvant dans la couronne représentent la moitié des ménages, mais 80% de l'artificialisation.
-   Les communes hors des aires d'attractions artificialisent relativement beaucoup plus que leur poids relatif en matière de ménages - *plus de 2 fois plus*.
...
-   Les quatre communes centres (Cambrai, Caudry, Le Cateau-Cambrésis et Solesmes) se trouvent assez logiquement à la droite de la droite grisée, elles n'artificialisent proportionnellement moins que leur poids relatif en ménages ;
-   Busigny, Les Rues-des-Vignes, Caudry et Cambrai ont artificialisé environ autant de surfaces liée à l'habitat mais ne disposent pourtant pas du même poids en matière de nombre de ménages ;
...
La situation est - comme déjà évoquée à ma maille communale - très différente de l'artificialisation liée à l'activité. On note de nombreuses poches d'artificialisation réparties sur l'ensemble des pôles du SCoT.

L'ensemble des ces carreaux se situent en périphérie des tâches urbaines, comme le détaille les cartes suivantes reprenant les mêmes données avec un zoom sur les communes les plus actives.
...
Une commune ayant gagné des emplois et artificialisé se trouvera alors au-dessus de l'axe des abscisses, comme Raillencourt-Saint-Olle. À l'inverse, une commune ayant perdu des emplois et artificialisé se trouvera sous cet axe. Plus la commune se trouve en bas et à droite, plus elle a artificialisé ET perdu des emplois.
...
-   Cambrai se distingue nettement en matière de perte d'emploi et une faible artificialisation liée à l'activité ;
-   Caudry se distingue en matière d'artificialisation liée à l'activité et avec très peu d'emplois perdus ;
...
-   Peu de communes ont à la fois peu artificialisé et gagné relativement des emplois : c'est le cas de Raillencourt-Sainte-Odile, Ligny en Cambrésis, Tilloy-lez-Cambrai, etc.
...
Afin de rendre la carte lisible, seules les communes ayant artificialisé plus d'un Ha sur la période sont gardées (soit 25% des communes dans le cas d'espèce).
...
-   L'*Ouest* de Cambrai et le Nord ont beaucoup artificialisé ;

-   Toutefois, cette carte fait ressortir de grandes disparités dans ces dynamiques :

    -   Quelques communes connaissent une efficacité relativement élevée (environ 48 emplois par Ha artificialisé), alors que d'autres se stabilisent ;
    -   Cambrai et le Cateau-Cambrésis se distinguent par leur taux négatifs.
...
On ne note pas de réelle tendance. On peut toutefois avancer que les communes-centres, bénéficiant du plus fort taux d'emplois et n'artificialisant en moyenne moins que certaines communes de la couronne connaissent une grande variabilité liée à l'évolution du taux d'emplois.
...
Une commune ayant gagné des ménages et artificialisé se trouvera alors au-dessus de l'axe des abscisses, comme Cambrai ou Caudry. À l'inverse, une commune ayant perdu des ménages et artificialisé se trouvera sous cet axe. Plus la commune se trouve en bas et à droite, plus elle a artificialisé ET perdu des ménages.
...
-   On peut exclure les communes de Marcoing, Iwuy, Saulzoir qui ont gagné des ménages de manière conséquente. On note par ailleurs, comme les communes n'ayant pas gagné de ménage, une grande disparité dans les dynamiques avec Marcoing ayant consommé que 0,5 Ha pour 50 nouveaux ménages, alors que Saulzoir, 3 Ha pour 45 ménages.

-   Enfin Caudry et Cambrai ont consommé presque autant de surfaces pour accueillir une grande différence de ménages.
...
Afin de rendre la carte lisible, seules les communes ayant artificialisé plus d'un Ha sur la période sont gardées (soit plus de 75 % des communes dans le cas d'espèce).
...
-   3/4 des communes de la couronne ont une médiane très faible (mais positive) ;
-   Il y a - comme pour l'activité - une forte volatilité de l'efficacité des communes-centres ;
-   L'efficacité médiane des communes-centres et des communes de la couronne sont identiques.
